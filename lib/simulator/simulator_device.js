...
module.exports = function(app){
    ...
    async.waterfall([
        ...
        function(result, cb) {  
            api.getDeviceDescriptor(cb); //서비스 획득 API 호출
        },
        function(result, cb) {  
            if(result.code == 104){
                console.log(colors.yellow("no Device Descriptor in Platform Service"));     
                api.createDeviceDescriptor(cb); // 가상 서비스 생성 API 호출
            }
            else
                cb(null, result);
        },
